...
// Match the warning baseline of the published crate when compiled from this
// checked-in path dependency.
#![allow(unused_unsafe)]
...
impl<T: UserEvent> Context<T> {
  ...
  fn create_webview(
    ...
    let (creation_tx, creation_rx) = channel();
    ...
    // Registration is transactional: do not hand Tauri a dispatcher/label
    // until the platform WebView actually exists in the runtime window.
    creation_rx
      .recv()
  ...
  fn close(&self) -> Result<()> {
    ...
    send_user_message(
      ...
      Message::Webview(
        *self.window_id.lock().unwrap(),
        self.webview_id,
        WebviewMessage::Close(tx),
      ),
    )?;

    // Do not report closure until the platform-loop-owned wrapper has
    // actually been removed and dropped. This makes same-label close/open
    // ordering transactional across concurrent proxy senders.
  ...
  fn hide(&self) -> Result<()> {
    ...
    send_user_message(
      ...
      Message::Webview(
        *self.window_id.lock().unwrap(),
        self.webview_id,
        WebviewMessage::SetVisible(false, tx),
      ),
    )?;
    rx.recv().map_err(|_| Error::FailedToReceiveMessage)?
  }
  ...
  fn show(&self) -> Result<()> {
    ...
    send_user_message(
      ...
      Message::Webview(
        *self.window_id.lock().unwrap(),
        self.webview_id,
        WebviewMessage::SetVisible(true, tx),
      ),
    )?;
    rx.recv().map_err(|_| Error::FailedToReceiveMessage)?
  }
...
fn rc_is_exclusive<T>(value: &Rc<T>) -> bool {
...
fn reparent_webview_to_window(webview: &WebviewWrapper, window: &Window) -> wry::Result<()> {
  #[cfg(target_os = "macos")]
  {
    use wry::WebViewExtMacOS;
    webview.inner.reparent(window.ns_window() as _)
  }

  #[cfg(windows)]
  {
    webview.inner.reparent(window.hwnd())
  }
  ...
  {
    if let Some(container) = window.default_vbox() {
...
#[cfg(test)]
mod recovery_tests {
  use super::rc_is_exclusive;
  use std::rc::Rc;

  #[test]
  fn wrapper_teardown_waits_for_reentrant_clones() {
    let runtime_owner = Rc::new(());
    assert!(rc_is_exclusive(&runtime_owner));

    let callback_owner = runtime_owner.clone();
    assert!(!rc_is_exclusive(&runtime_owner));
  ...
  match message {
      ...
      if let Some((Some(window), webviews, has_children, window_event_listeners)) = w {
        match window_message {
          ...
          WindowMessage::ScaleFactor(tx) => {
            let _ = tx.send(window.scale_factor());
          }
          ...
          WindowMessage::InnerSize(tx) => {
            let _ =
              tx.send(PhysicalSizeWrapper(inner_size(&window, &webviews, has_children)).into());
          }
          WindowMessage::OuterSize(tx) => {
            let _ = tx.send(PhysicalSizeWrapper(window.outer_size()).into());
          }
          WindowMessage::IsFullscreen(tx) => {
            let _ = tx.send(window.fullscreen().is_some());
          }
          WindowMessage::IsMinimized(tx) => {
            let _ = tx.send(window.is_minimized());
          }
          WindowMessage::IsMaximized(tx) => {
            let _ = tx.send(window.is_maximized());
          }
          WindowMessage::IsFocused(tx) => {
            let _ = tx.send(window.is_focused());
          }
          WindowMessage::IsDecorated(tx) => {
            let _ = tx.send(window.is_decorated());
          }
          WindowMessage::IsResizable(tx) => {
            let _ = tx.send(window.is_resizable());
          }
          WindowMessage::IsMaximizable(tx) => {
            let _ = tx.send(window.is_maximizable());
          }
          WindowMessage::IsMinimizable(tx) => {
            let _ = tx.send(window.is_minimizable());
          }
          WindowMessage::IsClosable(tx) => {
            let _ = tx.send(window.is_closable());
          }
          WindowMessage::IsVisible(tx) => {
            let _ = tx.send(window.is_visible());
          }
          WindowMessage::Title(tx) => {
            let _ = tx.send(window.title());
          }
          WindowMessage::CurrentMonitor(tx) => {
            let _ = tx.send(window.current_monitor());
          }
          WindowMessage::PrimaryMonitor(tx) => {
            let _ = tx.send(window.primary_monitor());
          }
          WindowMessage::MonitorFromPoint(tx, (x, y)) => {
            let _ = tx.send(window.monitor_from_point(x, y));
          }
          WindowMessage::AvailableMonitors(tx) => {
            let _ = tx.send(window.available_monitors().collect());
          }
          #[cfg(any(
            target_os = "linux",
            target_os = "dragonfly",
          ...
          WindowMessage::GtkWindow(tx) => {
            let _ = tx.send(GtkWindow(window.gtk_window().clone()));
          }
          #[cfg(any(
            target_os = "linux",
            target_os = "dragonfly",
          ...
          WindowMessage::GtkBox(tx) => {
            if let Some(default_vbox) = window.default_vbox() {
              let _ = tx.send(GtkBox(default_vbox.clone()));
            }
          }
          #[cfg(target_os = "android")]
          WindowMessage::ActivityName(tx) => {
            let _ = tx.send(window.activity_name());
          }
          #[cfg(target_os = "ios")]
          WindowMessage::SceneIdentifier(tx) => {
            let _ = tx.send(window.scene_identifier());
          }
          ...
          WindowMessage::Theme(tx) => {
            let _ = tx.send(map_theme(&window.theme()));
          }
          WindowMessage::IsEnabled(tx) => {
            let _ = tx.send(window.is_enabled());
          }
          WindowMessage::IsAlwaysOnTop(tx) => {
            let _ = tx.send(window.is_always_on_top());
          }
          // Setters
    ...
    Message::Webview(window_id, webview_id, webview_message) => {
      ...
      if let WebviewMessage::Reparent(new_parent_window_id, tx) = webview_message {
        // Keep the runtime registry transactional with the native hierarchy. A
        // failed AppKit/GTK/Win32 attachment must not silently drop the only
        // event-loop-owned wrapper or leave the dispatcher waiting forever.
        let removed = windows
          .0
          .borrow_mut()
          .get_mut(&window_id)
          .and_then(|window| {
            let old_parent = window.inner.clone()?;
            window
              .webviews
              .iter()
              .position(|webview| webview.id == webview_id)
              .map(|index| (index, old_parent, window.webviews.remove(index)))
          });

        let result = if let Some((old_index, old_parent, webview)) = removed {
          let new_parent = windows
            .0
            .borrow()
            .get(&new_parent_window_id)
            .and_then(|window| window.inner.clone());

          let reparent_result = new_parent
            .as_deref()
            .ok_or(wry::Error::MessageSender)
            .and_then(|window| reparent_webview_to_window(&webview, window));

          if reparent_result.is_ok() {
            if let Some(new_parent_window) = windows.0.borrow_mut().get_mut(&new_parent_window_id) {
              new_parent_window.webviews.push(webview);
              Ok(())
            } else {
              // The target disappearing synchronously is not expected, but
              // restore both native and registry ownership if it ever does.
              let rollback_result = reparent_webview_to_window(&webview, &old_parent);
              if let Some(old_parent_window) = windows.0.borrow_mut().get_mut(&window_id) {
                old_parent_window
                  .webviews
                  .insert(old_index.min(old_parent_window.webviews.len()), webview);
              }
              if let Err(error) = rollback_result {
                log::error!("failed to roll back webview reparent: {error}");
              }
              Err(Error::FailedToSendMessage)
            }
          } else {
            if let Err(error) = &reparent_result {
              log::error!("failed to reparent webview: {error}");
            }
            let rollback_result = reparent_webview_to_window(&webview, &old_parent);
            if let Some(old_parent_window) = windows.0.borrow_mut().get_mut(&window_id) {
              old_parent_window
      ...
      if let WebviewMessage::Close(tx) = webview_message {
        // Remove the event-loop-owned wrapper before acknowledging.
        // Dropping it performs Wry's native teardown; no new child with
        // the same application label may be published before this ack.
        let removed = windows
          .0
          .borrow_mut()
          .get_mut(&window_id)
          .and_then(|window| {
            window
              ...
              .iter()
              .position(|webview| webview.id == webview_id)
              .map(|index| (index, window.webviews.remove(index)))
          });
        let result = match removed {
          Some((_index, webview)) if rc_is_exclusive(&webview.inner) => {
            // This Drop is the one that tears down Wry and clears its context.
            drop(webview);
            Ok(())
          }
          Some((index, webview)) => {
            // A synchronously re-entrant Close can observe an outer handler's
            // temporary clone. Reinsert the wrapper and retry after that outer
            // native callback releases its reference.
            if let Some(window) = windows.0.borrow_mut().get_mut(&window_id) {
              window
                .webviews
                .insert(index.min(window.webviews.len()), webview);
              Err(Error::FailedToSendMessage)
            } else {
              // The parent vanished during shutdown; dropping the last runtime
              // owner is complete teardown, so let Tauri remove its label.
              drop(webview);
              Ok(())
            }
          }
      ...
      if let WebviewMessage::SetVisible(visible, tx) = webview_message {
        let webview = windows.0.borrow().get(&window_id).and_then(|window| {
          window
            .webviews
            .iter()
        ...
        let result = match webview {
          Some(webview) => webview.set_visible(visible).map_err(|error| {
            log::error!("failed to change webview visibility: {error}");
      ...
      if let Some((Some(window), Some(webview))) = webview_handle {
        match webview_message {
          ...
          WebviewMessage::SetVisible(_, _) => { /* handled transactionally above */ }
          WebviewMessage::Print => {
            let _ = webview.print();
          }
          WebviewMessage::Close(_) => { /* handled transactionally above */ }
          WebviewMessage::SetBounds(bounds) => {
          ...
          WebviewMessage::Url(tx) => {
            let url = webview
              .url()
              .map_err(|_| Error::FailedToSendMessage)
              .and_then(|url| url.parse().map_err(|_| Error::FailedToSendMessage));
            let _ = tx.send(url);
          }

          WebviewMessage::Cookies(tx) => {
            let _ = tx.send(webview.cookies().map_err(|_| Error::FailedToSendMessage));
          }
          ...
          WebviewMessage::IsDevToolsOpen(tx) => {
            let _ = tx.send(webview.is_devtools_open());
          }
        }
      }
    }
    Message::CreateWebview(window_id, handler, completion) => {
      let window = windows
        .0
        .borrow()
        .get(&window_id)
        .map(|w| (w.inner.clone(), w.focused_webview.clone()));
      let result = if let Some((Some(window), focused_webview)) = window {
        match handler(&window, CreateWebviewOptions { focused_webview }) {
          Ok(webview) => {
            if let Some(w) = windows.0.borrow_mut().get_mut(&window_id) {
              ...
              // Dropping `webview` removes the just-created native child.
              Err(Error::WindowNotFound)
            }
          }
          Err(e) => {
      ...
      // A vanished caller is ordinary cancellation; never panic on the event
      // loop while reporting a platform construction result.
    ...
    Message::CreateRawWindow(window_id, handler, sender) => {
      ...
      if let Ok(window) = builder.build(event_loop) {
        ...
        let _ = sender.send(Ok(Arc::downgrade(&window)));
      } else {
...
"#,
  ...
  let mut web_context_store = context
    .main_thread
    .web_context
    .lock()
    .unwrap_or_else(|poisoned| poisoned.into_inner());
  let is_first_context = web_context_store.is_empty();
  ...
  let entry = web_context_store.entry(web_context_key.clone());
  ...
  // Keep the WebContext mutex guard outside the unwind boundary. If platform
  // construction panics, the store remains usable and the same label can be
  // retried after the parent view reattaches.
  let build_result = std::panic::catch_unwind(AssertUnwindSafe(|| match kind {
    #[cfg(not(any(
      target_os = "windows",
      target_os = "macos",
      target_os = "ios",
  ...
  let webview = match build_result {
    Ok(Ok(webview)) => webview,
    Ok(Err(error)) => {
      web_context.referenced_by_webviews.remove(&label);
      #[cfg(not(any(
        target_os = "linux",
        target_os = "dragonfly",
        target_os = "freebsd",
      ...
      if web_context.referenced_by_webviews.is_empty() {
        // End the entry borrow before pruning the failed, unused
        // context from the store.
        let _ = web_context;
        web_context_store.remove(&web_context_key);
      }
      return Err(Error::CreateWebview(Box::new(error)));
    }
    Err(_) => {
      web_context.referenced_by_webviews.remove(&label);
      #[cfg(not(any(
      ...
      return Err(Error::CreateWebview(Box::new(std::io::Error::other(
        "platform webview construction panicked",
      ))));
    }
  };
