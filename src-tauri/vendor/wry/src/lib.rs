...
// The crates.io build suppresses these inherited macOS SDK compatibility
// warnings. A checked-in path dependency is compiled as a local crate, so keep
// the same warning baseline while retaining all correctness lints.
...
impl WebViewExtMacOS for WebView {
  ...
  fn ns_window(&self) -> Retained<NSWindow> {
    self.webview.ns_window.borrow().clone()
  }
