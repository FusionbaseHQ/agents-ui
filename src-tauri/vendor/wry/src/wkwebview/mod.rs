...
/// Rolls back the global protocol state if native construction exits through
/// an error, Rust panic, or Objective-C exception before `InnerWebView` owns it.
struct PendingWebViewStateRegistration {
  id: String,
  generation: u64,
  committed: bool,
}
...
#[cfg(target_os = "macos")]
fn contain_native_work<T>(work: impl FnOnce() -> Result<T>) -> Result<T> {
  std::panic::catch_unwind(AssertUnwindSafe(|| {
    objc2::exception::catch(AssertUnwindSafe(work)).map_err(|_| Error::ObjectiveCException)?
  }))
  .map_err(|_| Error::NativeCallbackPanicked)?
}

impl Drop for PendingWebViewStateRegistration {
  fn drop(&mut self) {
    if !self.committed {
      let mut state = WEBVIEW_STATE
        .write()
...
pub(crate) struct InnerWebView {
  ...
  #[cfg(target_os = "macos")]
  // Retain the owning window independently of transient view attachment.
  // `reparent` updates this only after the new attachment succeeds.
  pub(crate) ns_window: RefCell<Retained<NSWindow>>,
  #[allow(dead_code)]
  is_child: bool,
...
impl InnerWebView {
  ...
  fn new_ns_view(
    ...
    // Safety: objc runtime calls are unsafe. On macOS the builder executes on
    // Tauri's event-loop stack, so contain both Rust panics and Objective-C
    // exceptions here and return an ordinary construction error to the caller.
    let build = || unsafe {
      #[cfg(target_os = "macos")]
      let ns_window = ns_view.window().ok_or(Error::DetachedParentView)?;
      ...
      let state_generation = next_state_generation();
      WEBVIEW_STATE
        .write()
        .unwrap_or_else(|poisoned| poisoned.into_inner())
        .insert(
      ...
      let mut state_registration = PendingWebViewStateRegistration {
        id: webview_id.clone(),
        generation: state_generation,
        committed: false,
      };
      ...
      let mut w = Self {
        ...
        #[cfg(target_os = "macos")]
        ns_window: RefCell::new(ns_window.clone()),
        data_store,
        pending_scripts,
...
r#"Object.defineProperty(window, 'ipc', {
  value: Object.freeze({postMessage: function(s) {window.webkit.messageHandlers.ipc.postMessage(s);}})
      ...
      #[cfg(target_os = "macos")]
      {
        let attached_window = ns_view.window().ok_or(Error::DetachedParentView)?;
        if Retained::as_ptr(&attached_window) != Retained::as_ptr(&ns_window) {
          return Err(Error::DetachedParentView);
        }
        if is_child {
          ns_view.addSubview(&webview);
          let Some(child_window) = webview.window() else {
            webview.removeFromSuperview();
            return Err(Error::DetachedParentView);
          };
          if Retained::as_ptr(&child_window) != Retained::as_ptr(&ns_window) {
            webview.removeFromSuperview();
            return Err(Error::DetachedParentView);
          }
        } else {
      ...
      #[cfg(target_os = "macos")]
      {
        // Activation and delegate installation may synchronously re-enter
        // AppKit. Validate the exact attachment once more at the commit point.
        let attached_window = w.webview.window().ok_or(Error::DetachedParentView)?;
        if Retained::as_ptr(&attached_window) != Retained::as_ptr(&ns_window) {
          w.webview.removeFromSuperview();
          return Err(Error::DetachedParentView);
        }
        if is_child {
          let parent = w.webview.superview().ok_or(Error::DetachedParentView)?;
          if !std::ptr::eq::<NSView>(&*parent, ns_view) {
            w.webview.removeFromSuperview();
            return Err(Error::DetachedParentView);
          }
        }
      ...
      state_registration.committed = true;
      Ok(w)
    };

    #[cfg(target_os = "macos")]
    {
      let constructed = std::panic::catch_unwind(AssertUnwindSafe(|| {
        objc2::exception::catch(AssertUnwindSafe(build)).map_err(|_| Error::ObjectiveCException)
      }))
      .map_err(|_| Error::NativeCallbackPanicked)?;
      constructed?
    }
  ...
  pub fn eval(&self, js: &str, callback: Option<impl Fn(String) + Send + 'static>) -> Result<()> {
      ...
      unsafe {
        ...
        if let Some(callback) = callback {
          let handler = block2::RcBlock::new(move |val: *mut AnyObject, _err: *mut NSError| {
            // WebKit invokes this block across an Objective-C ABI boundary.
            // Contain both native exceptions and user callback panics here;
            // the initiating eval call has already returned by this point.
            let _ = std::panic::catch_unwind(AssertUnwindSafe(|| {
              let _ = objc2::exception::catch(AssertUnwindSafe(|| unsafe {
                ...
                if let Some(value) = val.as_ref() {
                  if let Ok(json_data) = NSJSONSerialization::dataWithJSONObject_options_error(
                    value,
                    objc2_foundation::NSJSONWritingOptions::FragmentsAllowed,
                  ) {
                    let json_string = NSString::alloc();
                    if let Some(json_string) =
                      NSString::initWithData_encoding(json_string, &json_data, NSUTF8StringEncoding)
                    {
    ...
    #[cfg(target_os = "macos")]
    return contain_native_work(evaluate);
  ...
  pub fn reload(&self) -> crate::Result<()> {
    #[cfg(target_os = "macos")]
    return contain_native_work(|| {
      // Safety: Objective-C work is contained above.
  ...
  fn navigate_to_url(&self, url: &str, headers: Option<http::HeaderMap>) -> crate::Result<()> {
    // Reject malformed input before asking Foundation to construct an NSURL;
    // Foundation is still allowed to return nil for a syntactically valid URL.
    let _ = url::Url::parse(url)?;
    let navigate = || -> crate::Result<()> {
      // Safety: native work is contained on macOS below.
      unsafe {
        let url = NSURL::URLWithString(&NSString::from_str(url)).ok_or(Error::MessageSender)?;
    ...
    #[cfg(target_os = "macos")]
    return contain_native_work(navigate);

    #[cfg(not(target_os = "macos"))]
    navigate()
  }

  fn navigate_to_string(&self, html: &str) -> crate::Result<()> {
    let navigate = || -> crate::Result<()> {
      // Safety: native work is contained on macOS below.
      unsafe {
        self
  ...
  pub fn print_with_options(&self, _options: &PrintOptions) -> crate::Result<()> {
    ...
    unsafe {
      ...
      if can_print {
        ...
        // Launch the modal
        let window = self.webview.window().ok_or(Error::DetachedParentView)?;
  ...
  pub fn bounds(&self) -> crate::Result<Rect> {
    #[allow(unused_unsafe)]
    unsafe {
      let parent = self.webview.superview().ok_or(Error::DetachedParentView)?;
  ...
  pub fn set_bounds(&self, #[allow(unused)] bounds: Rect) -> crate::Result<()> {
    ...
    if self.is_child {
      ...
      unsafe {
        let parent_view = self.webview.superview().ok_or(Error::DetachedParentView)?;
        let frame = CGRect {
  ...
  pub(crate) fn reparent(&self, window: *mut NSWindow) -> crate::Result<()> {
    let attempted_add = Cell::new(false);
    let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
      objc2::exception::catch(AssertUnwindSafe(|| unsafe {
        let window = window.as_ref().ok_or(Error::DetachedParentView)?;
        let content_view = window.contentView().ok_or(Error::DetachedParentView)?;
        attempted_add.set(true);
        content_view.addSubview(&self.webview);
        let attachment_matches = self
          .webview
          .window()
          .is_some_and(|attached_window| std::ptr::eq::<NSWindow>(&*attached_window, window))
          && self.webview.superview().is_some_and(|attached_parent| {
            std::ptr::eq::<NSView>(&*attached_parent, &*content_view)
          });
        if !attachment_matches {
          self.webview.removeFromSuperview();
          return Err(Error::DetachedParentView);
        }
        *self.ns_window.borrow_mut() = window.retain();
        Ok(())
      }))
      .map_err(|_| Error::ObjectiveCException)?
    }))
    .map_err(|_| Error::NativeCallbackPanicked)?;
    if result.is_err() && attempted_add.get() {
      let _ = std::panic::catch_unwind(AssertUnwindSafe(|| {
        let _ = objc2::exception::catch(AssertUnwindSafe(|| unsafe {
          self.webview.removeFromSuperview();
        }));
...
pub fn url_from_webview(webview: &WKWebView) -> Result<String> {
  let url_obj = unsafe { webview.URL() }
    .ok_or_else(|| Error::Io(std::io::Error::other("native webview URL is unavailable")))?;
  let absolute_url = unsafe { url_obj.absoluteString() }.ok_or_else(|| {
    Error::Io(std::io::Error::other(
      "native webview URL string is unavailable",
    ))
  })?;
...
impl Drop for InnerWebView {
  fn drop(&mut self) {
    ...
    if state
      .get(&self.id)
      .is_some_and(|entry| entry.generation == self.state_generation)
    {
      state.remove(&self.id);
    }
    drop(state);
    ...
    #[cfg(target_os = "macos")]
    {
      // Drop runs inside Tauri's event-loop message handler. Keep each native
      // cleanup independent: an exception removing the script handler must not
      // skip detaching the view (and vice versa), and neither Rust nor
      // Objective-C unwinding may cross the platform boundary.
      if let Some(ipc_handler) = self.ipc_handler_delegate.take() {
        let _ = std::panic::catch_unwind(AssertUnwindSafe(|| {
          let _ = objc2::exception::catch(AssertUnwindSafe(|| unsafe {
            let ipc = ns_string!(IPC_MESSAGE_HANDLER_NAME);
            // This decreases the retain count and releases the handler closure.
            ipc_handler
              .ivars()
              .controller
      ...
      let _ = std::panic::catch_unwind(AssertUnwindSafe(|| {
        let _ = objc2::exception::catch(AssertUnwindSafe(|| unsafe {
          // Make an orphaned layer inert even if detach itself later fails.
      ...
      let _ = std::panic::catch_unwind(AssertUnwindSafe(|| {
        let _ = objc2::exception::catch(AssertUnwindSafe(|| unsafe {
          // Remove the webview from its current superview before dropping it.
          self.webview.removeFromSuperview();
        }));
      }));

      // Preserve Wry's upstream retain balancing independently from native
      // cleanup so an Objective-C exception cannot suppress it.
      let _ = self.webview.retain();
...
mod wake_safety_tests {
  use super::*;

  #[test]
  fn native_work_converts_rust_panics_to_errors() {
    let result = contain_native_work::<()>(|| panic!("injected native callback panic"));
    assert!(matches!(result, Err(Error::NativeCallbackPanicked)));
  }

  #[test]
  fn pending_protocol_registration_rolls_back_on_failure() {
    let id = "__wry_pending_registration_rollback_test__".to_string();
    WEBVIEW_STATE
      .write()
      .unwrap_or_else(|poisoned| poisoned.into_inner())
      .insert(
    ...
    {
      let _pending = PendingWebViewStateRegistration {
        id: id.clone(),
        generation: 1,
        committed: false,
      };
    }
    assert!(!WEBVIEW_STATE
      .read()
      .unwrap_or_else(|poisoned| poisoned.into_inner())
      .contains_key(&id));
  }
  ...
  #[test]
  fn stale_registration_guard_cannot_remove_new_same_label_state() {
    let id = "__wry_stale_registration_generation_test__".to_string();
    WEBVIEW_STATE
      .write()
      .unwrap_or_else(|poisoned| poisoned.into_inner())
      .insert(
    ...
    {
      let _stale = PendingWebViewStateRegistration {
        id: id.clone(),
        generation: 1,
        committed: false,
      };
    ...
    let mut state = WEBVIEW_STATE
      .write()
      .unwrap_or_else(|poisoned| poisoned.into_inner());
    assert_eq!(state.get(&id).map(|entry| entry.generation), Some(2));
    state.remove(&id);
  }
}
