...
fn contain_delegate_callback(callback: impl FnOnce()) {
  let _ = std::panic::catch_unwind(AssertUnwindSafe(|| {
    let _ = objc2::exception::catch(AssertUnwindSafe(callback));
  }));
}
...
define_class!(
  ...
  unsafe impl WKNavigationDelegate for WryNavigationDelegate {
    ...
    fn did_finish_navigation(&self, webview: &WKWebView, navigation: &WKNavigation) {
      contain_delegate_callback(|| did_finish_navigation(self, webview, navigation));
    }

    #[unsafe(method(webView:didCommitNavigation:))]
    fn did_commit_navigation(&self, webview: &WKWebView, navigation: &WKNavigation) {
      contain_delegate_callback(|| did_commit_navigation(self, webview, navigation));
    }
    ...
    fn navigation_download_action(
      ...
      contain_delegate_callback(|| navigation_download_action(self, webview, action, download));
    }
    ...
    fn navigation_download_response(
      ...
      contain_delegate_callback(|| navigation_download_response(self, webview, response, download));
    }

    #[unsafe(method(webViewWebContentProcessDidTerminate:))]
    fn web_content_process_did_terminate(&self, webview: &WKWebView) {
      contain_delegate_callback(|| web_content_process_did_terminate(self, webview));
    }
  }
);
