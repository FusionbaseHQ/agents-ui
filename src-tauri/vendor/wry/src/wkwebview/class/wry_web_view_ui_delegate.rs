...
fn contain_ui_callback<T>(callback: impl FnOnce() -> T) -> Option<T> {
...
impl Drop for NewWindow {
  fn drop(&mut self) {
    let _ = contain_ui_callback(|| unsafe {
...
define_class!(
  ...
  unsafe impl WKUIDelegate for WryWebViewUIDelegate {
    ...
    fn run_file_upload_panel(
      ...
      let urls = contain_ui_callback(|| unsafe {
      ...
      .flatten();
      let pointer = urls.as_ref().map(Retained::as_ptr).unwrap_or_else(null);
      let _ = contain_ui_callback(|| unsafe {
