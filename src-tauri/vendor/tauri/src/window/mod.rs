...
impl<R: Runtime> Window<R> {
  ...
  pub fn add_child<P: Into<Position>, S: Into<Size>>(
    ...
    self.run_on_main_thread(move || {
      // Wry/AppKit construction runs on this event-loop stack. Contain any
      // dependency panic here so a temporarily detached parent view is
      // reported to the caller instead of unwinding across the macOS loop.
      let res = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        webview_builder.build(window_, position, size)
      }))
      .unwrap_or_else(|_| {
        Err(crate::Error::Io(std::io::Error::other(
          "child webview construction panicked on the platform thread",
        )))
      });
      // The invoking thread can disappear while the event-loop task is
      // pending; channel closure is an ordinary cancellation, not a panic.
      let _ = tx.send(res);
    })?;
    rx.recv()
