...
```
  ...
  pub(crate) fn build(
    ...
    let initially_visible = self.initially_visible;
    ...
    // Creation and this acknowledged visibility update execute on the same
    // platform task, before AppKit's event loop can render the child. Roll
    // back both runtime and manager state if the child cannot be hidden.
...
/// Webview attributes.
impl<R: Runtime> WebviewBuilder<R> {
  /// Sets whether the child webview is visible when this builder returns.
  /// The update is acknowledged in the same platform task as construction.
  #[must_use]
