...
#[cfg(any(target_os = "macos", target_os = "ios"))]
use early_termination::{
  publish_webview_and_take_early_termination, record_early_web_content_termination,
};
...
pub struct WebviewManager<R: Runtime> {
  ...
  /// Termination callbacks can synchronously re-enter while Wry is still
  /// constructing a native webview, before it is published in `webviews`.
  /// Preserve those labels and deliver them immediately after publication.
  #[cfg(any(target_os = "macos", target_os = "ios"))]
  pub(crate) early_web_content_terminations: Mutex<HashSet<String>>,
...
impl<R: Runtime> WebviewManager<R> {
  ...
  fn prepare_pending_webview<M: Manager<R>>(
    ...
    if pending.on_web_content_process_terminate_handler.is_none() {
        ...
        .is_some()
      {
        ...
        app_manager_
          .webview
          .early_web_content_terminations
          .lock()
        ...
        pending
          ...
          .replace(Box::new(move || {
            let webview = record_early_web_content_termination(
              &app_manager_.webview.early_web_content_terminations,
              &label_,
              || app_manager_.get_webview(&label_),
            );
  ...
  pub(crate) fn attach_webview(
    ...
    #[cfg(any(target_os = "macos", target_os = "ios"))]
    let terminated_before_publication = publish_webview_and_take_early_termination(
      &self.early_web_content_terminations,
      webview.label(),
      || {
        self
          .webviews_lock()
    ...
    self
      ...
      .insert(webview.label().to_string(), webview.clone());
    #[cfg(any(target_os = "macos", target_os = "ios"))]
    if terminated_before_publication {
      if let Some(on_web_content_process_terminate) = &self.on_web_content_process_terminate {
