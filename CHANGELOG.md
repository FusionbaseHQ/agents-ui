...
- **UI/UX overhaul (phase 1)**:
  ...
  - **Automatic update notifications**: the app quietly checks the GitHub latest release shortly after launch and every 6 hours; a new version raises a one-time toast (View → Updates dialog) and lights the status-bar "Update available" chip. No infrastructure beyond publishing a GitHub release with a version tag; offline/rate-limit failures stay silent.
